...
class Entity {
  ...
  parseMesh(meshOfs: number) {
    ...
    for (let i = 0; i < nbSegments; i++) {
      ...
      if (
        this.bones[boneIndex] &&
        this.bones[boneParent] &&
        !this.bones[boneIndex].parent
      ) {
  ...
  parseAnimation = (keyFrameValuesOfs: number, keyFramesOfs: number) => {
    ...
    const keyFrameDict = this.readKeyFrameValues(
      keyFrameValuesOfs,
      keyFramesOfs
    );
  ...
  readFace(faceCount: number, isQuad: boolean, localIndices: WeightedVertex[]) {
